...
app.post('/auth', (req, res) => {
    ...
    let query = '';
    if (role === 'on') {
        query = `SELECT password FROM hospital_users WHERE hospital_username = ?`;
    } else {
...
app.get('/hospital_dashboard', (req, res) => {
    if (req.session.loggedin && req.session.role === 'on') {
        const query = `
            ...
            LEFT JOIN diseases d ON p.diseaseid = d.diseaseidno
            WHERE h.hospitalid = ${req.body.id} AND p.currentstatus = 'Under treatment';
        `;
